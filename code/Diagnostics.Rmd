...
data$RATING = data$RATING - mean(data$RATING, na.rm = TRUE) #centering around mean

lmod_trans = lm(PACKAGE ~ YEARS.OLD + INDUSTRY + INDIA.HQ + TOTAL_EMPLOYEES + BRANCHES + RATING + I(RATING^2) + REVIEWS , data)
summary(lmod_trans)
plot(lmod_trans, 1)
```
...
write.csv(transformed_data, "../data/indian_companies_transformed.csv", row.names = TRUE)


lmod_final = lm(PACKAGE ~ YEARS.OLD + INDUSTRY + INDIA.HQ + TOTAL_EMPLOYEES + BRANCHES + RATING + I(RATING^2) + REVIEWS, transformed_data)
