...
```{r}
lmod = lm(PACKAGE ~ YEARS.OLD + INDUSTRY + INDIA.HQ + TOTAL_EMPLOYEES + BRANCHES + RATING + I(RATING^2) + REVIEWS, comp)
summary(lmod)
```
...
out = step(null, scope = list(lower=~1, upper=formula(lmod)), direction = "both", trace = FALSE)
...
```{r echo=FALSE }
mean_rating = 3.746872
x_new = data.frame(REVIEWS = log(10000), BRANCHES = log(4), RATING=log(3)-log(mean_rating),
                   INDIA.HQ=factor("Bangalore / Bengaluru", levels = levels(comp$INDIA.HQ)), 
                   INDUSTRY=factor("Software Product", levels = levels(comp$INDUSTRY)))
...
x_new = data.frame(REVIEWS = log(10000), BRANCHES = log(4), RATING=log(3)-log(mean_rating),
                   INDIA.HQ=factor("Bangalore / Bengaluru", levels = levels(comp$INDIA.HQ)), 
                   INDUSTRY=factor("Education & Training", levels = levels(comp$INDUSTRY)))
